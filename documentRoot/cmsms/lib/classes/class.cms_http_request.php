...
class cms_http_request
{
    ...
    function clear()
    {
        ...
        $this->maxRedirect  = 3;
        $this->cookiePath   = TMP_CACHE_LOCATION.'/c'.md5(__CLASS__ . session_id()) . '.dat'; // by default, use a cookie file that is unique only to this session.
            ...
            if( in_array('curl',get_loaded_extensions()) ) {
                if( function_exists('curl_version') ) {
                    ...
                    if( isset($tmp['version']) ) {
                        if( version_compare($tmp['version'],'7.19.7') >= 0 ) $_curlgood = 1;
                    }
                }
            }
        }
